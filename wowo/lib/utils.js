...
// Menghasilkan kode aesthetic random untuk末尾 pesan
function getAestheticCode() {
  const prefix = ["H", "X", "K", "M", "R", "Z", "V", "L", "N", "Y"];
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  
  const prefixChar = prefix[Math.floor(Math.random() * prefix.length)];
  let code = prefixChar;
...
async function reply(m, text) {
  ...
  // Tambahkan kode aesthetic di akhir pesan
  const aestheticCode = getAestheticCode();
